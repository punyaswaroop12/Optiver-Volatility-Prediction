...
def calculate_wap(df, rank="1"):
    ...
    """
    return (
        df[f"bid_price{rank}"] * df[f"ask_size{rank}"]
        + df[f"bid_size{rank}"] * df[f"ask_price{rank}"]
    ) / (df[f"bid_size{rank}"] + df[f"ask_size{rank}"])


def calculate_inter_wap(df, rank="1"):
    return (
        df[f"bid_price{rank}"] * df[f"bid_size{rank}"]
        + df[f"ask_size{rank}"] * df[f"ask_price{rank}"]
    ) / (df[f"bid_size{rank}"] + df[f"ask_size{rank}"])
    pass
...
def window_stats(df, feature_dict, second_windows, additional_dfs=None):
    ...
    if additional_dfs:
        df_merged = df_merged.merge(
            additional_dfs, how="left", left_on="time_id_", right_on="time_id"
        )
    ...
    for window, temp_df in temp_dfs:
        df_merged = df_merged.merge(
            temp_df, how="left", left_on="time_id_", right_on=f"time_id__{window}"
        )
...
def get_stock_clusters(df, n_clusters=7):
    ...
    clusters = KMeans(n_clusters, random_state=cfg.random_state).fit(
        corr_pivoted.values
    )
    ...
    for i in range(n_clusters):
        groups.append(
            [x - 1] for x in (corr_pivoted.index + 1) * (clusters.labels_ == i) if x > 0
        )
    return groups
    pass
